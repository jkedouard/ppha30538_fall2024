...
alt.Chart(movies_url).mark_area().transform_calculate(
    NonUS_Gross='datum.Worldwide_Gross - datum.US_Gross' # making a new variable
).encode(
    alt.X('month(Release_Date):T'),
...
alt.Chart(movies_url).mark_circle().encode(
    alt.X('Rotten_Tomatoes_Rating:Q'),
    alt.Y('IMDB_Rating:Q')
).transform_filter('year(datum.Release_Date) < 1970') # filters down what shows up on the plot
```
...
alt.Chart(movies_url).mark_bar().transform_aggregate(
    ...
    alt.X('Average_Rating:Q').title("Average Rotten Tomatoes Rating"),
    alt.Y('Major_Genre:N').title("Film Genre")
)
```
...
alt.Chart(movies_url).mark_bar().transform_aggregate(
    ...
    alt.X('Gross:Q', axis = alt.Axis(labelAngle = 15)),
    alt.Y('Director:N', sort=alt.EncodingSortField(
        op='max', field='Gross', order='descending'
    ))
)
